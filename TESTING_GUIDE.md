...
- Нефракционные тикеры: при открытии и ребалансировке по ним выставляются ордера в целых штуках (qty), число акций берётся из последней рыночной цены Alpaca; если цены нет или денег меньше стоимости 1 бумаги — тикер пропускается с предупреждением. Фракционные торгуются как раньше по notional.
